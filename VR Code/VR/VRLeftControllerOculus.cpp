#include "StdAfx.h"
#include "VRLeftControllerOculus.h"
...
float VRLeftControllerOculus::angx = 0;
float VRLeftControllerOculus::angy = 0;
float VRLeftControllerOculus::angz = 0;
struct Point {
	double x;
	double y;
};
EntityId VRMovementComponentOculus::leftHand = -1;
Vec3 VRLeftControllerOculus::angVel = Vec3(0, 0, 0);

void PhysicalizeLeftControllerOculus(IEntity& entity) {
...
static void RegisterVRLeftControllerOculusComponent(Schematyc::IEnvRegistrar& registrar)
{
	Schematyc::CEnvRegistrationScope scope = registrar.Scope(IEntity::GetEntityScopeGUID());
	{
		Schematyc::CEnvRegistrationScope componentScope = scope.Register(SCHEMATYC_MAKE_ENV_COMPONENT(VRLeftControllerOculus));
		// Functions	
		{
		}
	}
}
CRY_STATIC_AUTO_REGISTER_FUNCTION(&RegisterVRLeftControllerOculusComponent)


void VRLeftControllerOculus::Initialize() {
	...
	VRMovementComponentOculus::leftHand = leftHand->GetId();
	PhysicalizeLeftControllerOculus(*leftHand);// physicalize hand for physics
...
Ang3 VRLeftControllerOculus::getAngles() {
	return Ang3(VRLeftControllerOculus::angx, VRLeftControllerOculus::angy, VRLeftControllerOculus::angz);
}

void VRLeftControllerOculus::MoveLiving(IPhysicalEntity& physicalEntity, Matrix34 world)
{
	pe_action_move move;
...
void VRLeftControllerOculus::ProcessEvent(const SEntityEvent& event) {
	...
	//	if (param == "LeftHoldEnd") {
	//		holdend = true;
	//	}
	//	else if (param == "LeftGeneralEnd") {
	//		generalEnd = true;
	//	}
	//}
	VRLeftControllerOculus::angx = xRot;
	VRLeftControllerOculus::angy = yRot;
	VRLeftControllerOculus::angz = zRot;
		...
		if (IHmdDevice* pDevice = pHmdManager->GetHmdDevice()) // Check, if a valid HMD device is connected
		{
			if (pDevice->GetClass() == EHmdClass::eHmdClass_Oculus) // Check, if the connected device is an Oculus device)
			{
				...
				if (pController->IsConnected(eHmdController_OculusLeftHand))
				{
					...
					/**
					Orients the Right controller relative to HMD Cam. Based it off of rotation.
					*/
					//-----------------------------------------------------------------------------------------------------------------------
					IEntity* body = gEnv->pEntitySystem->FindEntityByName("HMD Cam");
					HmdTrackingState cState = pController->GetLocalTrackingState(eHmdController_OculusLeftHand);
					...
					Matrix34 impulse = worldLocale - leftHand->GetWorldTM();//create an impulse scalar
					MoveLiving(*leftHand->GetPhysicalEntity(), impulse);// move right hand via impulse for physics
					leftHand->SetRotation(Quat(worldLocale));// reorient hand with new quat
					VRMovementComponentOculus::leftTriggerOculus = pController->GetTriggerValue(eHmdController_OculusLeftHand, static_cast<EKeyId>(eHmdController_OculusLeftHand + eKI_Motion_OculusTouch_L1));//update right trigger value
					bGripPressed = pController->GetTriggerValue(eHmdController_OculusLeftHand, static_cast<EKeyId>(eHmdController_OculusLeftHand + eKI_Motion_OculusTouch_L2)) >= .7; // check if grip is greater than 70%, then consider it pressed
					trigger = VRMovementComponentOculus::leftTriggerOculus >= .7 ? true : false;// check if trigger is greater than 70%, then consider it pressed
					//-----------------------------------------------------------------------------------------------------------------------
				}
			}
		}
...
Cry::Entity::EventFlags VRLeftControllerOculus::GetEventMask() const {
