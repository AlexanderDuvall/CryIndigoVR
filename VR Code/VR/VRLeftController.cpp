...
void VRLeftController::Initialize() {
	...
	PhysicalizeLeftController(*leftHand); // physicalize hand for physics
			...
			if (pDevice->GetClass() == EHmdClass::eHmdClass_OpenVR) // Check, if the connected device is an Oculus device)
			{
				...
				if (pController->IsConnected(eHmdController_OpenVR_1))
				{
					...
					*/
					...
					VRMovementComponent::leftTrigger = pController->GetTriggerValue(eHmdController_OpenVR_1, eKI_Motion_OpenVR_Trigger);//update right trigger value
					bGripPressed = pController->IsButtonPressed(eHmdController_OpenVR_1, eKI_Motion_OpenVR_Grip);//update grip values
					trigger = pController->GetTriggerValue(eHmdController_OpenVR_1, eKI_Motion_OpenVR_Trigger) >= .7 ? true : false;// check if trigger is greater than 70%, then consider it pressed
					//-----------------------------------------------------------------------------------------------------------------------
				}
			}
		}
