...
void PhysicalizeLiving(IEntity& entity) {
	...
	playerDimensions.sizeCollider = Vec3(0.01f, 0.01f, .01f);
...
void VRMovementComponent::Initialize() {
	...
	PhysicalizeLiving(*newEntity); //physicalize "body"
				...
				if (IHmdDevice* pDevice = pHmdManager->GetHmdDevice()) // Check, if a valid HMD device is connected
				{
					if (pDevice->GetClass() == EHmdClass::eHmdClass_OpenVR) // Check, if the connected device is an Oculus device)
					{
						...
						if (pController->IsConnected(eHmdController_OpenVR_1))
						{
							...
							//Check when user touches joystick at greater than 20%, and move if true
							...
							if (abs(vTouchPad.x) > 0.2 || abs(vTouchPad.y) > .2) {
								...
								Point p = VRtoRect(Direction); // camera direction vector
								double speed = m_speed * controllerVector;
								MoveLiving(*phys, p.x * speed, p.y * speed, -2); // move body in the direction player is looking
							}
							else {
								//idle position with gravity set to -6 m/s (-9.8 causes glitches and probs)
								MoveLiving(*phys, 0, 0, -6);
							}
						}
					}
				}
