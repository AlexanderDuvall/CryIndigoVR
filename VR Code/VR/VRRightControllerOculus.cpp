...
void VRRightControllerOculus::Initialize() {
	...
	PhysicalizeRightControllerOculus(*rightHand);// physicalize hand for physics
	mainCamera = gEnv->pSystem->GetViewCamera();

	//This is the Animation Component for VR. If you want to have animated hands, you can implement the actions here.
	//Just make sure you have a hand with animations (ofc).
			...
			if (pDevice->GetClass() == EHmdClass::eHmdClass_Oculus) // Check, if the connected device is an Oculus device)
			{
				...
				if (pController->IsConnected(eHmdController_OculusRightHand))
				{
					...
					*/
					...
					Matrix34 impulse = m - rightHand->GetWorldTM();//create an impulse scalar
					MoveLiving(*rightHand->GetPhysicalEntity(), impulse);// move right hand via impulse for physics
					rightHand->SetRotation(Quat(m));// reorient hand with new quat
					VRMovementComponentOculus::rightTriggerOculus = pController->GetTriggerValue(eHmdController_OculusRightHand, static_cast<EKeyId>(eHmdController_OculusRightHand + eKI_Motion_OculusTouch_L1));//update right trigger value
					bGripPressed = pController->GetTriggerValue(eHmdController_OculusRightHand, static_cast<EKeyId>(eHmdController_OculusRightHand + eKI_Motion_OculusTouch_L2)) >= .7;// check if grip is greater than 70%, then consider it pressed
					trigger = VRMovementComponentOculus::rightTriggerOculus >= .7 ? true : false;// check if trigger is greater than 70%, then consider it pressed
					//-----------------------------------------------------------------------------------------------------------------------
				}
			}
		}
