...
void VRRightController::Initialize() {
	...
	VRMovementComponent::rightHand = rightHand->GetId(); //physicalize right hand
			...
			if (pDevice->GetClass() == EHmdClass::eHmdClass_OpenVR) // Check, if the connected device is an Oculus device)
			{
				...
				if (pController->IsConnected(eHmdController_OpenVR_2))
				{
					...
					*/
					...
					Matrix34 newLocale = Matrix34::Create(Vec3(1, 1, 1), q, calcHandLocale); //hand locale
					...
					VRMovementComponent::rightTrigger = pController->GetTriggerValue(eHmdController_OpenVR_2, eKI_Motion_OpenVR_Trigger);//update right trigger value
					bGripPressed = pController->IsButtonPressed(eHmdController_OpenVR_2, eKI_Motion_OpenVR_Grip);//update grip values
					trigger = pController->GetTriggerValue(eHmdController_OpenVR_2, eKI_Motion_OpenVR_Trigger) >= .7 ? true : false; // check if trigger is greater than 70%, then consider it pressed
					//-----------------------------------------------------------------------------------------------------------------------
				}
			}
		}
