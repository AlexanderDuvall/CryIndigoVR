...
void beamLeftOculus(IEntity * body, IEntity* hand) {
...
void VRLeftControllerOculus::Initialize() {
	...
	PhysicalizeLeftControllerOculus(*leftHand);
		...
		if (IHmdDevice* pDevice = pHmdManager->GetHmdDevice()) // Check, if a valid HMD device is connected
		{
			if (pDevice->GetClass() == EHmdClass::eHmdClass_Oculus) // Check, if the connected device is an Oculus device)
			{
				...
				if (pController->IsConnected(eHmdController_OculusLeftHand))
				{
					...
					VRMovementComponentOculus::leftTriggerOculus = pController->GetTriggerValue(eHmdController_OculusLeftHand, static_cast<EKeyId>(eHmdController_OculusLeftHand + eKI_Motion_OculusTouch_L1));
					bGripPressed = pController->GetTriggerValue(eHmdController_OculusLeftHand, static_cast<EKeyId>(eHmdController_OculusLeftHand + eKI_Motion_OculusTouch_L2)) >= .7;
					trigger = VRMovementComponentOculus::leftTriggerOculus >= .7 ? true : false;
				}
			}
		}
	}
